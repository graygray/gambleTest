...
public class Banker {
    ...
    public static int firstReverseRoundResult;

    public static boolean isMoneyExceedFlag = false;
	...
	isReverse = false;
	firstReverseRoundResult = NA;
    }

    public static void deal() {
	// 亂數產生結果
	dice1 = 1 + (int) (generator.nextInt(6));
	dice2 = 1 + (int) (generator.nextInt(6));
	dice3 = 1 + (int) (generator.nextInt(6));
	// 豹子通殺
	...
	// 開大
	    ...
	    continuousSmallCounter = 0;
	    continuousSmallCounterReal = 0;
	    if(continuousBigCounter == 0){
		continuousBigCounter = 1;
	    }
	    if(continuousBigCounterReal == 0){
	    ...
	    continuousBigCounter = 0;
	    continuousBigCounterReal = 0;
	    if(continuousSmallCounter == 0){
		continuousSmallCounter = 1;
	    }
	    if(continuousSmallCounterReal == 0){
...
//	}
	...
	for (AbstractPlayer player : players) {
		...
		player.setBetResult(NA);
		previousResult = result;
	    }
	    
	    if (isMoneyExceedFlag) {
		isMoneyExceedFlag = false;
		continuousBigCounter = 0;
		continuousSmallCounter = 0;
	    }
	}
...
//	Log.e("gray", "dice3, point 6:" + dice3_6counter + ", op:" + (double) dice3_6counter / gameCounter);
	for (AbstractPlayer player : players) {
	    ...
	    if (MainActivity.playerIndex == 2) {
		MainActivity.logString += player.playerName + " bet big:" + ((Player2)player).betBigCounter + "\n";
		MainActivity.logString += player.playerName + " bet small:" + ((Player2)player).betSmallCounter + "\n";
	    }
	    MainActivity.logString += "win:" + player.winCounter + "\n";
	    MainActivity.logString += "lose:" + player.loseCounter + "\n";
	}
